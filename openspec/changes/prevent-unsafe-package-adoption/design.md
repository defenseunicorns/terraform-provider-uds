...
The guarded sequence is: back up state and deployment metadata; reconstruct values, variables, selected components, namespace override, and other package inputs; inspect and compare canonical rendering; keep the fail-closed provisional state in place until the operator is ready for an exclusive handoff window; remove only that Terraform state with `tofu state rm`; perform the eligible canonical deployment externally; verify canonical Zarf, Helm, Kubernetes, and application state; delete only the exact verified stale alias Zarf Secret; then import the canonical identity and review the resulting plan. No Terraform plan or apply may run between state removal and canonical import because the still-configured resource could otherwise be planned as a new canonical deployment. `tofu destroy`, alias package removal, and old-release uninstall are not substitutes for state removal or cleanup.
...
- [The alias guard can block unrelated changes while assessment is in progress] -> Keep the fail-closed state until migration is ready rather than risk an unintended create after early state removal; schedule an exclusive handoff window for state removal through canonical import.
