...
- [ ] 8.1 Author `templates/guides/aliased-package-migration.md.tmpl` with the four independent state layers, required backups and deployment-input reconstruction, per-release classification, ordinary-chart and raw-manifest paths, exact-object takeover limits, stop conditions, handoff verification, exact stale alias cleanup, canonical import, partial-failure handling, and destructive-operation warnings; verify the guide satisfies every migration-guidance scenario in `specs/package-adoption-safety/spec.md`.
- [ ] 8.2 Link the migration guide from `templates/resources/package.md.tmpl`, run `uds run generate --no-progress`, and verify `docs/guides/aliased-package-migration.md` and the Registry-relative resource link are generated correctly.
- [ ] 8.3 Manually execute the published ordinary-chart path on a disposable cluster using a temporary synthetic package without committing a migration-only fixture; verify same-release handoff, canonical Zarf identity, workload health, exact alias cleanup, canonical import, and post-import plan behavior.
...
- [ ] 8.5 Run `uds run generate --no-progress`, the repository documentation and lint checks, `git diff --check`, and strict OpenSpec validation; verify all commands pass and generated files are current.
