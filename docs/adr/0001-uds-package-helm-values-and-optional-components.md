...
- **Order matters.** Index 0 is the base layer; each subsequent element overrides the previous. Objects deep-merge (Helm convention); scalar and list values replace. Implementation: the provider writes each string to a temporary `.yaml` file in order and calls `value.ParseFiles()` from the Zarf SDK, passing the returned `value.Values` to `DeployOptions.Values`. No custom merge logic is implemented in the provider.
...
**Not interchangeable with cli-next.** cli-next uses Go `text/template` syntax (`{{ .vars.domain }}`) because it is a Go application with no access to Tofu's HCL evaluation engine. The provider delegates rendering entirely to Tofu (`${var.domain}`, `templatefile()`, etc.). Users migrating between tools must rewrite template syntax — this is an intentional trade-off that keeps the provider idiomatic for Tofu while avoiding a second variable system.
