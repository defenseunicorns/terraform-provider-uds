...
- **Order matters.** Index 0 is the base layer; each subsequent element overrides the previous. Objects deep-merge (Helm convention); scalar and list values replace. Implementation: the provider creates an isolated temporary directory per package deploy invocation, writes each string to a sequentially named `.yaml` file within it, and calls `value.ParseFiles()` from the Zarf SDK, passing the returned `value.Values` to `DeployOptions.Values`. A per-invocation directory is required to prevent file collisions between concurrent package deployments. The directory must be cleaned up immediately after `value.ParseFiles()` returns — including on deploy failure — to prevent accumulation across apply runs and to avoid exposing sensitive values content on the filesystem. The temp directory used must not be a location that CI runners archive or persist between runs. No custom merge logic is implemented in the provider.
