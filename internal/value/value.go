...
func ParseFiles(ctx context.Context, paths []string, _ ParseFilesOptions) (_ Values, err error) {
	...
	start := time.Now()
	defer func() {
		tflog.Debug(ctx, "values parsing complete", map[string]any{
	...
	tflog.Debug(ctx, "parsing values files", map[string]any{"paths": paths})
	for _, path := range paths {
...
func parseLocalFile(ctx context.Context, path string) (Values, error) {
	...
	defer func(f *os.File) {
		if closeErr := f.Close(); closeErr != nil {
			...
			tflog.Warn(ctx, "failed to close file", map[string]any{
				"path":  path,
